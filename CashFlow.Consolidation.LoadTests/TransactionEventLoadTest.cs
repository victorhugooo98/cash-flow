...
public class TransactionEventLoadTest
{
    ...
    public async Task LoadTest_TransactionEventProcessing_ShouldHandleHighVolume()
    {
        ...
        for (var i = 0; i < numberOfTransactions; i++)
        {
            ...
            transactionTasks.Add(Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        _output.WriteLine($"Transaction creation failed: {response.StatusCode} - {await response.Content.ReadAsStringAsync()}");
                    }
                }
                catch (Exception ex)
                {
        ...
        await Task.Delay(5000); // Give some time for events to be processed
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            try
            {
                ...
                {
                    var balance = await balanceResponse.Content.ReadFromJsonAsync<dynamic>();
                    var totalCredits = (decimal)balance.GetProperty("totalCredits").GetDecimal();
                    var totalDebits = (decimal)balance.GetProperty("totalDebits").GetDecimal();
                    var closingBalance = (decimal)balance.GetProperty("closingBalance").GetDecimal();
                    
                    _output.WriteLine($"Balance found: Credits={totalCredits}, Debits={totalDebits}, Closing={closingBalance}");
                    ...
                    if (Math.Abs(totalCredits - expectedCredits) < 0.01m && 
                        Math.Abs(totalDebits - expectedDebits) < 0.01m)
                    {
                        successfulBalanceChecks = 1;
                        break;
                    }
                    
                    _output.WriteLine($"Balance not yet consistent. Expected Credits={expectedCredits}, Debits={expectedDebits}");
                }
                ...
                {
                    _output.WriteLine($"Balance check failed: {balanceResponse.StatusCode} - {await balanceResponse.Content.ReadAsStringAsync()}");
                    break;
                }
            }
            catch (Exception ex)
        ...
        // Assertions
        Assert.True(successfulTransactions >= numberOfTransactions * 0.95, "At least 95% of transactions should be created successfully");
